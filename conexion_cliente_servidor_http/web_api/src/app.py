...
def actualizar_codigo_puerta(codigo_puerta: int):
    ...
    """
    puerta = database.session.query(database.Doors).filter(database.Doors.id_puerta == codigo_puerta).first()
...
def edit_user(rfid):
    ...
    user = database.session.query(database.Usuarios).filter(database.Usuarios.rfid == rfid).first()
...
def edit_door(id_puerta):
    ...
    door = database.session.query(database.Doors).filter(database.Doors.id_puerta == id_puerta).first()
...
def access_log():
    ...
    access_logs = database.session.query(database.AccessLog, database.Usuarios) \
        .join(database.Usuarios, database.AccessLog.rfid == database.Usuarios.rfid) \
        .offset(offset) \
        .limit(limit) \
        .with_entities(
            database.AccessLog.id_puerta.label('id_puerta'),
            database.AccessLog.rfid.label('rfid_usuario'),
            database.AccessLog.timestamp.label('fecha_acceso'),
            database.Usuarios.nombre.label('nombre')
        ) \
        .all()
...
def verificar_acceso():
    ...
    puerta: database.Doors = database.session.query(database.Doors).filter_by(id_puerta=puerta).first()
    ...
    user = database.session.query(database.Usuarios).filter_by(rfid=rfid).first()
    ...
    access_log = database.AccessLog(id_puerta=puerta, rfid=rfid, acceso_concedido=acceso_concedido)

    # Add the access log to the database and commit the changes
    database.session.add(access_log)
    database.session.commit()
...
def inicializar_puerta():
    ...
    if puerta:
        ...
        return "Invalid request", 400
    # Find the user with the given RFID in the "Usuarios" table
    puerta = database.session.query(database.Doors).filter_by(id_puerta=num_puerta).first()
    if not puerta:
